...
Defines the class 'Track' to contain an individual iceberg track along with methods and properties.
...
class Specs:
    """
    Class that holds specifications for a specific beacon model
    """
...
class Track:
    ...
    """
    def __init__(self, data_file, reader="standard", model=None, track_start=None, track_end=None):
        """
        Read the track raw or standardized data.

        Note the default is to read standardized data. 
        ...
        self.data = reader_function(self.datafile)   
        
        # if these are given, then add them, otherwise they will be empty        
        self.model = model
    ...
    def calc_stats(self):
        ...
        self.beacon_id = self.data.beacon_id[0]
        ...
        # this only works after processing
    ...
    def clean(self, Specs):
        ...
        # Latitude
        self.sdf.loc[
            (self.sdf["latitude"] >= specs.latitude_max.iloc[0])
            | (self.sdf["latitude"] <= specs.latitude_min.iloc[0]),
            "latitude",
        ] = np.nan
    
        # Longitude
        self.sdf.loc[
            (self.sdf["longitude"] >= specs.longitude_max.iloc[0])
            | (self.sdf["longitude"] <= specs.longitude_min.iloc[0])
            | (self.sdf["longitude"] == 0),
            "longitude",
        ] = np.nan
    
        # Air temperature
        self.sdf.loc[
            (self.sdf["temperature_air"] >= specs.temperature_air_max.iloc[0])
            | (self.sdf["temperature_air"] <= specs.temperature_air_min.iloc[0]),
            "temperature_air",
        ] = np.nan
    
        # Internal temperature
        self.sdf.loc[
            (self.sdf["temperature_internal"] >= specs.temperature_internal_max.iloc[0])
            | (self.sdf["temperature_internal"] <= specs.temperature_internal_min.iloc[0]),
            "temperature_internal",
        ] = np.nan
    
        # Surface temperature
        self.sdf.loc[
            (self.sdf["temperature_surface"] >= specs.temperature_surface_max.iloc[0])
            | (self.sdf["temperature_surface"] <= specs.temperature_surface_min.iloc[0]),
            "temperature_surface",
        ] = np.nan
    
        # Pressure
        self.sdf.loc[
            (self.sdf["pressure"] >= specs.pressure_max.iloc[0])
            | (self.sdf["pressure"] <= specs.pressure_min.iloc[0]),
            "pressure",
        ] = np.nan
    
        # Pitch
        self.sdf.loc[
            (self.sdf["pitch"] >= specs.pitch_max.iloc[0])
            | (self.sdf["pitch"] <= specs.pitch_min.iloc[0]),
            "pitch",
        ] = np.nan
    
        # Roll
        self.sdf.loc[
            (self.sdf["roll"] >= specs.roll_max.iloc[0])
            | (self.sdf["roll"] <= specs.roll_min.iloc[0]),
            "roll",
        ] = np.nan
    
        # Heading
        self.sdf.loc[
            (self.sdf["heading"] >= specs.heading_max.iloc[0])
            | (self.sdf["heading"] <= specs.heading_min.iloc[0]),
            "heading",
        ] = np.nan
    
        # Satellites
        self.sdf.loc[
            (self.sdf["satellites"] >= specs.satellites_max.iloc[0])
            | (self.sdf["satellites"] <= specs.satellites_min.iloc[0]),
            "satellites",
        ] = np.nan
    
        # Battery voltage
        self.sdf.loc[
            (self.sdf["voltage"] >= specs.voltage_max.iloc[0])
            | (self.sdf["voltage"] <= specs.voltage_min.iloc[0]),
            "voltage",
        ] = np.nan
    
        # Drop all rows where datetime_data, latitude or longitude is nan
        self.sdf.dropna(subset=["datetime_data", "latitude", "longitude"], inplace=True)
    
        self.sdf = self.sdf.round(
            {
                "temperature_air": 2,
    ...
    def track_order(self):
        ...
        self.sdf.sort_values(["datetime_data", "loc_accuracy"], inplace=True)
        # look for repeated values
        # sdf_dup = sdf.loc[sdf.duplicated(subset=["datetime_data"], keep=False)] # all lines
        sdf_dup = self.sdf.loc[
            self.sdf.duplicated(subset=["datetime_data"], keep="last")
        ]  # keep last dup
